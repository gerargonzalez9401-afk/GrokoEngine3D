...
/// <list type="bullet">
/// <item><b>Linear</b>: las texturas/colores de entrada se decodifican de sRGB a
/// lineal antes de iluminar (PBR físicamente correcto), y el resultado final se
/// vuelve a codificar a sRGB/gamma para la pantalla. Es el flujo recomendado y
/// el que usan los motores modernos por defecto.</item>
/// <item><b>Gamma</b>: las texturas NO se decodifican — la iluminación opera
/// directamente sobre los valores "tal cual", pero el resultado sí se codifica a
/// sRGB/gamma para la pantalla (igual que Linear) para que la imagen no quede
/// oscura. Es el flujo "clásico": más barato y menos correcto físicamente, con un
/// look más brillante/contrastado. Se ofrece para comparar y elegir el que
/// prefieras. Lo único que cambia respecto a Linear es el decode de entrada.</item>
/// </list>
/// </summary>
...
/// en Unity (Project Settings → Quality). Controla la resolución de los shadow
/// maps de luces direccionales, focos y luces puntuales — más resolución implica
/// sombras más nítidas y con menos "escalera" en los bordes, a costa de VRAM y
/// tiempo de generación por frame.
/// </summary>
...
internal sealed partial class SceneViewportRenderer : IDisposable
{
    ...
    // Lista de (texturePath, startVertex, count) para dibujar por grupos de textura
    private readonly List<(string? Tex, int Start, int Count, ParticleBlendMode Blend, bool Soft, float SoftRange)> _particleRanges = new();
    ...
    private void BuildMeshBounds(GameObject obj, MeshFilter filter, ParsedMesh mesh, bool selected, Matrix4 transform)
    {
        ...
        // ── Skinning: si el objeto tiene un Animator y la malla trae pesos de huesos,
        // deformamos los vértices por CPU (forzando el camino de vértices, no el GPU draw). ──
        ...
        // Caja de selección: en mallas skinned el AABB real son los vértices deformados
        // (espacio mundo, ya con la escala del rig). El min/max estático está en cm sin esa
        // escala, así que daría una caja gigante. Solo se calcula si hay que dibujarla.
        ...
        // Clave barata del caché de skinning CPU: identidad del Animator + su PoseVersion.
        // Sustituye al hash de las 52 matrices por frame (mismo valor ⟺ misma pose).
        ...
        if (mesh.Submeshes.Count > 1)
        {
            ...
            {
                if (filter.SubmeshIndex >= 0 && i != filter.SubmeshIndex) continue; // hijo: solo su parte
        ...
        else
        {
            ...
            {
                AddSolidRange(texturePath, maps.NormalMapPath, maps.RoughnessMapPath, maps.MetallicMapPath, shaderGraphPath, shaderGraphProperties, shaderGraphTextures, () => AddMeshTriangles(mesh, transform, scale, color, meshPath, material, emission, 0, mesh.Positions.Length / 3));
            }
        }
        ...
        // Los vértices skinned ya están en espacio mundo; el AABB estático sí necesita el transform.
        if (!useSkinnedBounds)
            for (int i = 0; i < corners.Length; i++)
                corners[i] = Vector3.TransformPosition(corners[i], transform);
    ...
    {
        // Buscar en cache (evita re-indexar el ParsedMesh cada frame)
        ...
        {
            // Limitar tamaño del cache para evitar memory leak con muchos meshes/importScales distintos
        ...
        // Aplicar transform y color a los vértices cacheados (solo el rango de esta sub-malla)
        var v2 = cached.Verts;
    ...
    // CPU skinning: deforma cada vértice por las matrices de hueso (Offset*WorldHueso) y sus
    // pesos, produciendo posición/normal ya en espacio mundial. Vértices sin peso usan 'transform'.
    // AABB en espacio MUNDO de la malla deformada (los skin matrices llevan a mundo).
    // Se usa para dibujar la caja de selección del modelo skinned con su tamaño real.
        ...
        Vector4 color, Vector4 material, Vector4 emission, int vertexStart, int vertexCount, int skinKey)
    {
        ...
        for (int v = from; v < to; v++)
        {
            ...
            {
                // Vértice sin pesos o resultado no válido: transform normal del objeto.
    ...
    // Clave del caché de rango skinned por CPU. Antes hasheaba las 52 matrices de hueso
    // CADA frame (832 floats + string) y, al animar, nunca acertaba. Ahora usa skinKey
    // (identidad del Animator + PoseVersion): O(1), acierta con pose quieta y deja de
    // malgastar CPU al animar.
...
""";
    ...
    private readonly record struct DynamicMeshDraw(CachedGpuMesh Mesh, Matrix4 World, int Start, int Count, string? TexturePath, string? NormalMapPath, string? RoughnessMapPath, string? MetallicMapPath, Vector4 Color, Vector4 Material, Vector4 Emission);
